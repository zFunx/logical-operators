...
const createOperand = (operand, variables, ops) => {
    if (operand.val?.toString()) {
        ...
        // return <Operation result={ops[operand.op].} operator={ops[operand.op].operator} operands={ops[operand.op].operands} variables={variables} ops={ops} />
        return <Operation {...ops[operand.op]} ops={ops} variables={variables} />
    }
}
